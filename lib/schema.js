...
module.exports = class Schema {
    constructor (description) {
        if (Array.isArray(description)) {
            validateSchema(description[0])
            this.schema = description[0]
            this.isArray = true
        } else {
    ...
    validate (object) {
        if (!(object instanceof Object)) {
            return { valid: false, error: new Error('Argument should be an object or array') };
        }
        ...
        if (this.isArray) {
            if (!isArray) {
                return {
                    valid: false,
                    field: key, 
                    error: new Error(`Argument should be an array`)
                }
            }
            object.map((item, index) => {
                for (let key in this.schema) {
                    if (this.schema.hasOwnProperty(key)) {
                        if (!isValidValue(this.schema[key], item[key])) {
                            err = { valid: false, field: key, error: new Error(`Invalid value at field "${key}"`) };
                        }
                    }
                }
            })
